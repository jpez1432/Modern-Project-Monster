...
CTerrain::CTerrain(void) : Dimensions(0), MinHeight(256.0f)
{
...
bool CTerrain::Load(CPodPool &PodPool, std::string Filename, std::string Info)
{

    int Offset;

    Filename.insert(0, "Data\\");
    PodPool.FindFile(Filename, Dimensions, Offset);
    ...
    Info.insert(0, "Data\\");
    PodPool.FindFile(Info, Dimensions, Offset);

    if (Offset == 0 && !Config->Game.Files) {
        return false;
    }

    Info.insert(0, RootDir + ResourceDir);
    StreamIn.open(Info.c_str(), std::ios::binary | std::ios::in);
...
//            Result += TriNormals[0][IndexAt((x - 1),(z - 1))];
//            Result += TriNormals[1][IndexAt((x - 1),(z - 1))];
//
//            Result += TriNormals[0][IndexAt((x - 1), z)];
////            Result += TriNormals[1][IndexAt((x - 1), z)];
//
//            Result += TriNormals[0][IndexAt(x, z)];
//            Result += TriNormals[1][IndexAt(x, z)];
//
////            Result += TriNormals[0][IndexAt(x, (z - 1))];
//            Result += TriNormals[1][IndexAt(x, (z - 1))];
//
//            Normals[x + Dimensions * z] = glm::normalize(Result);
            ...
            Normals[z + Dimensions * x] = glm::normalize(Result);
...
const int CTerrain::IndexAt(int X, int Z)
{
    return int (((Z + Dimensions) % Dimensions) + ((X + Dimensions) % Dimensions) * Dimensions);
}

const int CTerrain::GetTextureIndex(int X, int Z)
{
    return TextureInfo[IndexAt(X, Z)].Index + ((TextureInfo[IndexAt(X, Z)].Rotation & 1) << 8);
}

const int CTerrain::GetTextureRotation(int X, int Z)
{
    return int(TextureInfo[IndexAt(X, Z)].Rotation);
}

const glm::vec3 CTerrain::GetHeight(int X, int Z, float Scale)
{
    return glm::vec3(X, float(HeightmapData[IndexAt(X, Z)] / Scale), Z);
}

glm::vec3 CTerrain::GetHeightScaled(int X, int Z)
{
    return glm::vec3(float(X), (HeightmapData[IndexAt(Z, X)] / 16.0f), float(Z));
}
