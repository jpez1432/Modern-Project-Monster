...
//        }
        ...
        if (Config->Graphics.Fullscreen) {
            ...
            glfwSetWindowMonitor(GLWindow, Monitors[Config->Graphics.Monitor], 0, 0, Width, Height, Config->Graphics.RefreshRate);
    ...
    if (Config->General.EditMode) {
        ...
        if (Key == GLFW_KEY_F1 && Action == GLFW_PRESS) {
            ...
            Config->Render.Fog = !Config->Render.Fog;
...
void MouseScrollCallback(GLFWwindow* Window, double xOffset, double yOffset)
{
    ...
    if (Config->Camera.Radius > 5.0f) {
        Config->Camera.Radius = 5.0f;
    }

    if (Config->Camera.Radius < 0.1f) {
        Config->Camera.Radius = 0.1f;
    }
...
bool Create(std::string ResDir, std::string CfgFile, std::string AppRoot)
{
    ...
    ResourceDir = ResDir;
    InterfaceFile = RootDir + ResourceDir + "Interface.cfg";
    ConfigFile = RootDir + ResourceDir + CfgFile;

    Logger = new CLogger(RootDir + ResourceDir + "Application.log");
...
//	glfwWindowHint(GLFW_OPENGL_PROFILE, GLFW_OPENGL_CORE_PROFILE);
    ...
    if (Config->Graphics.DoubleBuffer) {
    ...
    if (Config->Graphics.Monitor > Cnt) {
        ...
        Logger->Error("Invalid Monitor Selection, Defaulting...", true, true, false);
    ...
    if (Config->Graphics.Fullscreen) {
        ...
        GLWindow = glfwCreateWindow(Config->Graphics.Width, Config->Graphics.Height, GetInfo().c_str(), Monitors[Config->Graphics.Monitor], NULL);
    ...
    if (Config->Input.Joystick) {
        ...
        if (Input->Create(GLFW_JOYSTICK_1)) {
            ...
            Logger->Error("No Joystick Found, Disabling...", false, false, false);
            Config->Input.Joystick = false;
        }
    }
    ...
    if (Render->Create(*PodPool, *Track, *Vehicle)) {
...
bool Destroy(void)
{
    ...
    if (Vehicle) {
        ...
        Logger->Text("Unregistered Vehicle Client From ODE Server");

//        delete Vehicle;
//        Vehicle = NULL;
        Logger->Text("Destroyed ODE Vehicle Object");
    }

    if (Physics::CServer::GetInstance()->Destroy()) {
        Logger->Text("Destroyed ODE Physics Server Object");
    } else {
        Logger->Error("Error Destroying ODE Physics Server Object");
    }
...
//        Loop = Create("Resources\\", "Configuration.bin", argv[0]);
        Loop = Create("..\\Resources\\", "Configuration.bin", argv[0]);
        ...
        do {
            ...
            if (!Config->General.Paused) {
                Physics::CServer::GetInstance()->Update(Timer.GetDelta());
            }

            Render->Scene(Timer.GetDelta());
